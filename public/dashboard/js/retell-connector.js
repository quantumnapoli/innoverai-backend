...
const RETELL_CONFIG = {
    ...
    AGENT_ID: (typeof window !== 'undefined' && window.RETELL_CONFIG && window.RETELL_CONFIG.AGENT_ID) || "agent_0c73985e38f6110dbec24596c1", // Agente specifico
...
class RetellConnector {
        ...
        try {
            ...
            if (this.agentId && this.agentId.trim() !== '') {
                requestBody.filter_criteria = {
                    agent_id: [this.agentId]  // DEVE essere un array!
                };
        ...
        try {
            ...
            if (typeof getRetellSimulator === 'function') {
                ...
                const simulatedCalls = await simulator.listCalls({
                    agent_id: this.agentId ? [this.agentId] : undefined,  // Array per consistenza con API
                    call_type: 'inbound',
    ...
    async insertCallsToDatabase(calls) {
        try {
            ...
            // Prefer window.API_BASE_URL (populated from /config) otherwise fallback
            const apiUrl = (typeof window !== 'undefined' && window.API_BASE_URL)
                ? window.API_BASE_URL
                : (window.location.hostname === 'innoverai.com')
                    ? 'https://innoverai-production-06cb.up.railway.app'
                    : 'http://localhost:8080';
            console.log('🔗 Using API URL for inserts:', apiUrl);
            ...
            for (const call of calls) {
                try {
                    ...
                    if (!response.ok) {
                        ...
                        console.warn(`⚠️ Errore inserimento chiamata ${call.call_id}:`, response.statusText, errorData);
                    } else {
                        const result = await response.json();
                        if (result.created) {
                            console.log(`✅ Chiamata ${call.call_id} creata`);
                        } else if (result.updated) {
                            console.log(`✅ Chiamata ${call.call_id} aggiornata`);
                        }
                    }
                } catch (error) {
            ...
            console.log(`✅ ${calls.length} chiamate processate nel database`);
